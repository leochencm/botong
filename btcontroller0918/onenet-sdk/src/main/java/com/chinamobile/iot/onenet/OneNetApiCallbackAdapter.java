...
class OneNetApiCallbackAdapter implements Callback {
    ...
    public void onResponse(Call call, Response response) throws IOException {
        if (response.isSuccessful()) {
            ...
            byte[] buffer;
            if(responseBody.contentType().toString().equals("application/octet-stream")) {
                ...
                for (int i=0;i<buffer.length;i++)
                {
                    if((int)buffer[i]<0)
                    {
                        x=buffer[i];
                        x=x+256;
                        responseString+=String.valueOf((char) x);
                    }
                    else responseString+=String.valueOf((char) buffer[i]);
                }
            }
            else
            {
