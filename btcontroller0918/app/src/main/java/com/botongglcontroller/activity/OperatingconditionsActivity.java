...
//                    }
                ...
                case 5:
                    //刷新数据
                    if(isfirst){hideLoadingDialog();isfirst=false;}
...
//                int ord;
    ...
    private void postOperation(String function, String order, final String toasttrue, final String toastfalse, final int select) {
        try {
            ...
            if(function=="power")sendCommand(Sendcommand.devicePower(select), select,"16386",toasttrue,toastfalse);
            if(function=="run")sendCommand(Sendcommand.deviceRun(select),select,"16387",toasttrue,toastfalse);
            if(function=="hand")sendCommand(Sendcommand.deviceHand(select),select,"16388",toasttrue,toastfalse);
            ...
            while (_run) {
                try {
                    if(_run) {
                        ...
                        if(setflag>0&&_run){getDataPoint(str, dt);setflag=0;}
                    }
                    count++;
                            ...
                            try {
                               if(dt==0&&json.getString("current_value").equals("0")||dt==255&&json.getString("current_value").equals("255"))
                               {
                                   ...
                                   if(streamid.equals("16386")){
                                       if(json.getString("current_value").equals("0"))remotelock.setChecked(false);
                                       else remotelock.setChecked(true);
                                       LemonBubble.showRight(OperatingconditionsActivity.this, toasttrue, 1000);
                                   }
                                   else if(streamid.equals("16387")){
                                       if(json.getString("current_value").equals("0"))operation.setChecked(false);
                                       else operation.setChecked(true);
                                       LemonBubble.showRight(OperatingconditionsActivity.this, toasttrue, 1000);
                                   }
                                   else if(streamid.equals("16388")){
                                       if(json.getString("current_value").equals("0"))shoudong.setChecked(false);
                                       else shoudong.setChecked(true);
                                       LemonBubble.showRight(OperatingconditionsActivity.this, toasttrue, 1000);
                                   }
                                   for (int i=0;i<list_para.size();i++)
                    ...
                    if (0 == errno) {
                        ...
                        for (int i = 0; i < list_para.size(); i++) {
                            ...
                            if(Integer.parseInt(list_para.get(i).getAddr_int())>=0x5000&&Integer.parseInt(list_para.get(i).getAddr_int())<=0x50ff) {
                                if(!list_para.get(i).getValue().equals("0"))list_alert.add(list_para.get(i));
                            }
