    ...
    BroadcastReceiver Refresh = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            for (int i = 0; i < list3.size(); i++) {
                ...
                else if (list3.get(i).getKind().equals("3")&&list3.get(i).getVisiable().equals("1")) list_analog.add(list3.get(i));
            }
            ...
            while (_run) {
                try {
                    if (_run) {
                        ...
                        if(setflag>0&&_run){getDataPoint(str, dt,select);setflag=0;}
                    }
                    count++;
        ...
        try {
           final Date start = new Date(System.currentTimeMillis());
            OneNetApi.querySingleDataStream(mDevice, streamid, new OneNetApiCallback() {
                            ...
                            try {
                                if(json.getString("current_value").equals(select))
                                {
                                    ...
                                    for(int i=0;i<list_analog.size();i++)
                                        if(list_analog.get(i).getAddr_int().equals(streamid))list_analog.get(i).setValue(json.getString("current_value"));
                                    hideLoadingDialog();
                ...
                @Override
                public void onFailed(Exception e) {mytr.setflag=1;
                }
