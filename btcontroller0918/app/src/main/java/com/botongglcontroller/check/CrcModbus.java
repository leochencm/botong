...
public class CrcModbus {
	...
	public static String DoCRCModBus(String num){
		...
		crc =  s.substring(2, 4).toUpperCase()+" "+s.substring(0, 2).toUpperCase();
		return crc;
	}

	public static byte[] getCRCModBus(  byte[] submitData) {
		byte[] s=new byte[submitData.length-2];
		for (int i = 0; i <submitData.length-2 ; i++) {
			s[i]=submitData[i];
		}
		String bt=  DoCRCModBus(Bytes.Bytes2HexString(s));
		String MSG = bt.replace(" ", "");
		byte[]  bb=Bytes.HexString2Bytes(MSG );
