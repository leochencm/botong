...
public class CounterView extends TextView {
    ...
    protected void init(Context context, AttributeSet attrs, int defStyleAttr,
        ...
        final TypedArray typedArray = context.obtainStyledAttributes(attrs,
                R.styleable.CounterView, defStyleAttr, defStyleRes);

        try {
            final CharSequence prefix = typedArray
                    .getText(R.styleable.CounterView_prefix);
            if (prefix != null) {
            ...
            final CharSequence suffix = typedArray
                    .getText(R.styleable.CounterView_suffix);
            if (suffix != null) {
            ...
            final CharSequence text = typedArray
                    .getText(R.styleable.CounterView_android_text);
            if (text != null) {
            ...
            this.timeInterval = (long) typedArray.getFloat(
                    R.styleable.CounterView_timeInterval, DEFAULT_INTERVAL);
            this.increment = typedArray.getFloat(
                    R.styleable.CounterView_incrementValue, DEFAULT_INCREMENT);
            this.startValue = typedArray.getFloat(
                    R.styleable.CounterView_startValue, 0f);
            this.endValue = typedArray.getFloat(
                    R.styleable.CounterView_endValue, 0f);
            this.autoStart = typedArray.getBoolean(
                    R.styleable.CounterView_autoStart, true);
            this.autoFormat = typedArray.getBoolean(
                    R.styleable.CounterView_formatText, true);

            final int type = typedArray.getInteger(
                    R.styleable.CounterView_type, 0);
            switch (type) {
                case 0:
        ...
        counter = new Counter(this, startValue, endValue, timeInterval,
                increment);
